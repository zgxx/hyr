...
--00_门店查询会员日负毛利品种，以及建议零售价
SELECT DISTINCT ISNULL(ST.name,'') AS 门店,P.code AS 编号,P.name AS 商品名称, P.standard AS 规格,
  CONVERT(NUMERIC(18,2),PXMD.costp) AS 成本价,CONVERT(NUMERIC(18,2),PXMD.retailPrice) AS 零售价,
  CASE WHEN p.OTCFlag > 0 
  THEN (CASE WHEN PXMD.retailPrice*0.95-PXMD.costp < 0 THEN CONVERT(NUMERIC(18,1),PXMD.costp/0.95) ELSE CONVERT(NUMERIC(18,1),PXMD.retailPrice) END)
  ELSE (CASE WHEN PXMD.retailPrice*0.85-PXMD.costp < 0 THEN CONVERT(NUMERIC(18,1),PXMD.costp/0.85) ELSE CONVERT(NUMERIC(18,1),PXMD.retailPrice) END) END
   AS 会员日不亏损建议零售价,
 PXMD.retailPrice-PXMD.costp AS 毛利,
