...
DELETE FROM PM_Detail WHERE billid = @BID95 AND p_id IN 
  (SELECT Product_ID FROM Products WHERE DELETED = 1 OR Isdir = 1 OR (OTCFlag = 0 AND ColdStore = 0) 
    ...
    AND Product_ID NOT IN (7310)) --7310代表新疆鹿角胶(茎鹿)(纸盒)不从会员日95折里删除
...
IN (SELECT Product_ID FROM Products WHERE Isdir = 1 OR DELETED = 1 OR Parent_id LIKE '000004000001%' 
   OR Product_ID IN (8000,8001,8456,19072))
...
WHERE FML95.P_ID = PMD.p_id AND PMD.vipDayQty <> 2 AND PMD.billid = @BID95
...
WHERE FML88.P_ID = PMD.p_id AND PMD.vipDayQty <> 2 AND PMD.billid = @BID88
