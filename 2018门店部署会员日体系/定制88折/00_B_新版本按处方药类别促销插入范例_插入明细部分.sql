...
(SELECT billid FROM PM_Index WHERE billnumber = 'CX-180101-00021')  --非处方88折
...
IF (@BID_hyr1+@BID_hyr88+@BID_hyr95+@BID_hyr98+@BID_fhyr1+@BID_fhyr98+@BID_tdpz) IS NULL 
--IF (ISNULL(@BID_hyr1,0)+ISNULL(@BID_hyr88,0)+ISNULL(@BID_hyr95,0)+ISNULL(@BID_hyr98,0)+ISNULL(@BID_fhyr1,0)+ISNULL(@BID_fhyr98,0)) > 0
BEGIN 
...
--会员日,非处方药88折的品种
SELECT DISTINCT P.Product_ID AS P_ID,P.u_id,ISNULL(PXMD.retailPrice,0) AS retailPrice, ISNULL(PXMD.retailPrice*0.88,0) AS VIPretailPrice,
ISNULL(PXMD.costp,0) AS costp,ISNULL(PXMD.VipPrice,0) AS VipPrice,
CONVERT(NUMERIC(18,4),(ISNULL(PXMD.retailPrice*0.88,0) - ISNULL(costp,0))/ISNULL(PXMD.retailPrice*0.88,9999)) AS MLL,  --折后毛利率
0.88 AS CLASS,1 AS type
...
AND (
  P_ID IN (SELECT p_id FROM PM_Detail WHERE billid IN (@BID_hyr1))   --2018 会员日 自选限购品种
  OR 
...
AND (
  P_ID IN (SELECT p_id FROM PM_Detail WHERE billid IN (@BID_fhyr1))  --2018 非会员日 选定打折品种
  OR 
  P_ID IN (SELECT p_id FROM PM_Detail WHERE billid IN (@BID_tdpz))   --剔除门店添加的商品特价促销品种
  OR 
  P_ID IN (SELECT p_id FROM PM_Detail WHERE billid IN (@BID_zbsj))   --剔除总部特定锁价品种
...
--删除会员日体系里，错误的品种
DELETE FROM PM_Detail WHERE billid = @BID_hyr88 AND P_ID NOT IN (SELECT p_id FROM zgxCxTemp C WHERE C.CLASS = 0.88 AND C.type = 1)
...
SELECT @BID_hyr88,C.P_ID,C.u_id,0,0,C.class,0,1,0,0,0,C.profit_rate,0
FROM zgxCxTemp C LEFT JOIN (SELECT p_id FROM PM_Detail WHERE billid IN (@BID_hyr88)) PMD ON PMD.p_id = C.P_ID
WHERE C.CLASS = 0.88 AND C.type = 1 AND PMD.p_id IS NULL
...
(SELECT PMD.p_id,CASE WHEN P.OTCFlag > 0 THEN 0.95 
 WHEN P.Parent_id LIKE '000004000001%' THEN 1 ELSE 0.88 END AS DIS 
