...
--删除会员日体系里，错误的品种
DELETE FROM PM_Detail WHERE billid = @BID_hyr85 AND P_ID NOT IN (SELECT p_id FROM ##CxTemp C WHERE C.CLASS = 0.85 AND C.type = 1)
DELETE FROM PM_Detail WHERE billid = @BID_hyr95 AND P_ID NOT IN (SELECT p_id FROM ##CxTemp C WHERE C.CLASS = 0.95 AND C.type = 1)
DELETE FROM PM_Detail WHERE billid = @BID_hyr98 AND P_ID NOT IN (SELECT p_id FROM ##CxTemp C WHERE C.CLASS = 0.98 AND C.type = 1)

--删除非会员日体系里，错误的品种
DELETE FROM PM_Detail WHERE billid = @BID_fhyr98 AND P_ID NOT IN (SELECT p_id FROM ##CxTemp C WHERE C.CLASS = 0.98 AND C.type = 0)
